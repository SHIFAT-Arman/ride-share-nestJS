...
src/auth/
  ...
  auth.controller.ts    // ✅ POST /v1/api/auth/login + POST /v1/api/auth/register/:userType
...
- Normalize the found record to a common shape and build the role claim:
  - rider → `role: 'rider'`
  - driver → `role: 'driver'`
  - admin → `role: admin.role` (`SUPER_ADMIN` / `ADMIN` / `SUPPORT_AGENT`)
...
- `AuthModule` imports `RiderModule`, `DriverModule`, `AdminModule` (with their services exported) to get the lookups — avoid circular imports by having the domain modules *not* import `AuthModule` (only use its exported guard).
...
### 7. Registration endpoints (decision point) — ✅ done (resolved: unified `POST /v1/api/auth/register/:userType` covering rider/driver/admin; old per-domain registers and `POST /v1/api/rider/create` removed; admin register still sends the welcome email)
- Keep per-domain register endpoints (`/auth/register` admin, `/driver/auth/register` driver) as-is, or fold them into the auth controller as `POST /v1/api/auth/register/{userType}`. Registration is more domain-specific than login (driver registration creates a status `INACTIVE` driver, admin sends a welcome email), so I'd keep them per-domain for now and only unify login.
...
- **Admin role values** (`SUPER_ADMIN` etc.) are uppercase strings while rider/driver are lowercase — the RolesGuard comparison should be case-exact; keep the `@Roles` values matching the claim strings.
...
│   └── send-email.dto.ts
└── admin-role.model.ts            // AdminRole: SUPER_ADMIN | ADMIN | SUPPORT_AGENT
```
